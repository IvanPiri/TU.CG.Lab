...
{
	Mesh::Mesh(
		const std::vector<Vertex>& vertices, const std::vector<unsigned>& indices,
			...
			: textures(std::move(textures))
	{
		...
		auto vb = std::make_unique<VertexBuffer>(
			&vertices[0], static_cast<unsigned>(vertices.size() * sizeof(Vertex)));
		vb->Bind();
		vb->SetAttributes({
	...
	void Mesh::Draw(const ShaderProgram& shader) const
	{
		...
		{
			shader.SetInt(
				"material." + textures[i].GetFileNameWithoutExtension(), i);
